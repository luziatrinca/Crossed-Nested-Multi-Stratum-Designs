...
# COMPOUND CRITERIA
KapD <- 1                            # weight for D
KapDP <- 0                           # weight for DP
KapLP <- 0                           # weight for CIs (AP)
KapL <- 0                            # weight for point estimation (A)
KapDF <- 0                           # weight for df efficiency (lof)
...
# SET candExternal <- NULL FOR THE FULL FACTORIAL AS CANDIDATE SET, OTHERWISE,  
# CREATE A MATRIX NAMED candExternal WITH THE REQUIRED TREATMENT SET
candExternal <- NULL
...
# SEARCH DESIGN
Dphase1 <- SearchTreat(N, Npar, W, Kappa, Ntries, K, Nlev, Levels=NULL, 
                           candExternal=NULL, model, Fnames, probDP, probLP, MC,
                           parallelTries, update.info)
plan(sequential)              # REQUIRED FOR PARALLEL RUNNING

#save(Dphase1, file="Dphase1.RData")  # TO STORE IN EXTERNAL FILE
...
# NUMBER OF 'BLOCKS' (UNITS IN PREVIOUS STRATUM, HERE IS 10 OVENS TIMES 3 BATCHES)
Nbloc <- 30     
...
# load("Dphase1.RData")   # IF NOT AVAILABLE IN THE WORKSPACE

# Because each row of previous design must be replicated more than
# Bsize (actually there are Nblocp "superblocks"), the rows are pre-replicated
# here
Nblocp <- 3
Xwhole <- matrix(apply(Dphase1$Xopt,1,Expand,Nblocp),nc=K1,byrow=TRUE)
...
update.info <- TRUE           # FOR SHOWING PROGRESS OF THE SEARCH
plan(multisession)
Dphase2 <- SS_Search(algorithm, inverse.updating, Kappa, Xwhole,Nbloc,Bsize,
...
#save(Dphase2, file="Dphase2.RData") # TO STORE IN EXTERNAL FILE
...
# TERMS IN THE MODEL TO ESTIMATE FROM THE DESIGN AT HIGHER STRATA                                                
...
# WEIGHTS FOR PARAMETERS (not used for D)                        #
# 1 FOR LINEAR AND INTERACTIONS, 1/4 FOR QUADRATIC  
W <- rep(1,Npar)
W[(K2+1):(2*(K2))] <- .25
W <- W/sum(W)

#load("Dphase2.RData")  # IF NOT AVAILABLE IN THE WORKSPACE
...
N <- nrow(Xfull)
row.names(Xfull) <- 1:N
MSS_D <- FinalAdjust(Xfull, N, NRow, Rowsize, NCol, Colsize, PLOTsize, K1, K2, model, 
                           Npar, W, Fnames, Kappa, probDP, probLP)
colnames(MSS_D$Xopt) <- c("Ovens", "Batches", Fnames)

save(MSS_D, file="MSS_D.RData") # TO STORE IN EXTERNAL FILE
