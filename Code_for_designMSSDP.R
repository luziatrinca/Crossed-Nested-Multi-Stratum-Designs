...
# NUMBER OF UNITS (THE DESIGN IN THIS STRATUM IS COMPLETELY RANDOMIZED IN 
# 10 MAIN PLOTS)
N <- 10        

# COMPOUND CRITERIA
KapD <- 0                            # weight for D
KapDP <- 1                           # weight for DP
...
# load("DPphase1.RData")    # IF NOT AVAILABLE IN THE WORKSPACE
...
# here
Nblocp <- 3
Xwhole <- matrix(apply(DPphase1$Xopt,1,Expand,Nblocp),nc=K1,byrow=TRUE)
...
update.info <- TRUE           # FOR SHOWING PROGRESS OF THE SEARCH
plan(multisession)
DPphase2 <- SS_Search(algorithm, inverse.updating, Kappa, Xwhole,Nbloc,Bsize,
...
# save(DPphase2, file="DPphase2.RData")
...
# load("DPphase2.RData")  # IF NOT AVAILABLE IN THE WORKSPACE
Xfull <- DPphase2$Xopt
...
N <- nrow(Xfull)
row.names(Xfull) <- 1:N
MSS_DP <- FinalAdjust(Xfull, N, NRow, Rowsize, NCol, Colsize, PLOTsize, K1, K2, model, 
                           Npar, W, Fnames, Kappa, probDP, probLP)
colnames(MSS_DP$Xopt) <- c("Ovens", "Batches", Fnames)

save(MSS_DP, file="MSS_DP.RData")
